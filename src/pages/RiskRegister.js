    ...
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 24px" }}>
      <div style={{ textAlign: "center", marginBottom: 40 }}>
        ...
        <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 36, fontWeight: 400, color: "#e8e4d9", margin: "0 0 10px" }}>
          AI Risk Register <em style={{ color: "#ffd460" }}>(RAID Log)</em>
        </h1>
      ...
      {!raid ? (
        <div style={{ background: "#111118", border: "1px solid #ffffff10", borderRadius: 16, padding: 40, maxWidth: 680, margin: "0 auto" }}>
          <div style={{ display: "grid", gap: 20 }}>
            {[
              { key: "project", label: "Project / Platform Name", ph: "e.g. Salesforce CRM Implementation — KIPP Texas" },
              { key: "type", label: "Project Type", ph: "e.g. SaaS Implementation, Digital Transformation" },
              { key: "timeline", label: "Project Timeline", ph: "e.g. 16 weeks, Q2–Q3 2026" },
              { key: "team", label: "Team / Stakeholders", ph: "e.g. 8-person cross-functional team: PM, IT, Training" },
            ].map(({ key, label, ph }) => (
              <div key={key}>
            ...
            <div>
              <label style={{ display: "block", fontSize: 11, letterSpacing: "0.15em", textTransform: "uppercase", color: "#8a8478", marginBottom: 7 }}>Additional Context (optional)</label>
              <textarea value={form.context} onChange={e => setForm(f => ({ ...f, context: e.target.value }))} placeholder="Any known risks, constraints, or special requirements..." rows={3}
                style={{ width: "100%", boxSizing: "border-box", background: "#0a0a0f", border: "1px solid #ffffff15", borderRadius: 8, padding: "11px 14px", color: "#e8e4d9", fontSize: 14, outline: "none", resize: "vertical" }} />
            </div>
          </div>
          {error && <div style={{ marginTop: 14, color: "#e94560", fontSize: 13, textAlign: "center" }}>{error}</div>}
          <button onClick={generate} disabled={loading || !form.project || !form.type}
            style={{ width: "100%", marginTop: 28, padding: 15, background: loading || !form.project ? "#ffffff10" : "linear-gradient(135deg,#ffd460,#f0b800)", border: "none", borderRadius: 10, color: loading || !form.project ? "#8a8478" : "#0a0a0f", fontSize: 14, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", cursor: loading || !form.project ? "not-allowed" : "pointer" }}>
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
            <div>
              <div style={{ fontSize: 11, color: "#ffd460", letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 6 }}>RAID Log Generated</div>
