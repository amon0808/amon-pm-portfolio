  ...
  if (step === "form") return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 24px" }}>
      <div style={{ textAlign: "center", marginBottom: 48 }}>
        ...
        <p style={{ color: "#8a8478", fontSize: 15, maxWidth: 500, margin: "0 auto" }}>Fill in your project details and get a complete 5-phase implementation plan in seconds.</p>
      </div>
      <div style={{ background: "#111118", border: "1px solid #ffffff10", borderRadius: 16, padding: 40, maxWidth: 680, margin: "0 auto" }}>
        <div style={{ display: "grid", gap: 20 }}>
          {[{k:"product",l:"Platform / Product Name"},{k:"industry",l:"Industry / Sector"},{k:"clientSize",l:"Client Organization Size"},{k:"timeline",l:"Implementation Timeline"},{k:"stakeholders",l:"Key Stakeholders"}].map(({k,l}) => (
        ...
        <button onClick={generate} disabled={loading||!form.product||!form.industry} style={{ width:"100%",marginTop:28,padding:15,background:loading||!form.product?"#ffffff10":"linear-gradient(135deg,#e94560,#c73652)",border:"none",borderRadius:10,color:loading||!form.product?"#8a8478":"#fff",fontSize:14,fontWeight:600,letterSpacing:"0.08em",textTransform:"uppercase",cursor:loading||!form.product?"not-allowed":"pointer" }}>
      ...
      {playbook.phases?.[activePhase] && (()=>{const p=playbook.phases[activePhase]; return (
        <div style={{ background:"#111118",border:"1px solid #ffffff10",borderRadius:16,overflow:"hidden",marginBottom:28 }}>
          <div style={{ background:"linear-gradient(135deg,#1a1a2e,#16213e)",borderBottom:"1px solid #e9456030",padding:"22px 28px",display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:12 }}>
            <div><div style={{ fontFamily:"'Playfair Display',serif",fontSize:20,marginBottom:5 }}>{ICONS[activePhase]} {p.name} Phase</div><div style={{ color:"#8a8478",fontSize:13 }}>{p.objective}</div></div>
            <div style={{ textAlign:"right" }}><div style={{ fontSize:10,color:"#e94560",letterSpacing:"0.15em",textTransform:"uppercase" }}>Duration</div><div style={{ fontSize:18,color:"#ffd460",marginTop:2 }}>{p.duration}</div></div>
          </div>
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr" }}>
            <div style={{ padding:"22px 26px",borderRight:"1px solid #ffffff10" }}>
              ...
              {p.tasks?.map((t,i)=><div key={i} style={{ display:"flex",gap:9,marginBottom:11,alignItems:"flex-start" }}><div style={{ width:19,height:19,minWidth:19,border:"1px solid #e9456060",borderRadius:4,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,color:"#e94560" }}>{i+1}</div><div style={{ color:"#c8c4b9",fontSize:13,lineHeight:1.5 }}>{t}</div></div>)}
            </div>
